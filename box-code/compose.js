...
const COMPOSE_SYSTEM = [
  ...
  "TWO-STAGE WORKFLOW: if a fact the instruction asks you to state genuinely cannot be found (e.g. no supplier ETA exists yet), set status='awaiting_input', set draft to an empty string, and put a precise question in questions_for_salesperson rather than inventing it. NEVER paper over a gap with filler such as 'we will get back to you'. Physical checks the salesperson must perform go in physical_checks. If every needed fact is in hand, set status='ready' with the complete draft and leave interim_draft empty.",
  "INTERIM DRAFT IS REQUIRED WHENEVER YOU HOLD: whenever status='awaiting_input', write interim_draft — a real, sendable email containing everything we CAN say, and only what we are highly confident is accurate. This is what the salesperson sees in the send box, so it must be safe to send exactly as written. Cover every part of the instruction our data settles, leave out anything you are not sure of, and do not mention, hedge or allude to the uncertain part at all — a shorter email that is certainly true beats a fuller one that might not be. Only when there is genuinely nothing we can say with confidence may interim_draft be empty. The withheld fact belongs in questions_for_salesperson, never in the interim.",
